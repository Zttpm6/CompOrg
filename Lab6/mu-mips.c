...
void help() {
    ...
    printf("cdump\t -- dump cache values\n");
...
uint32_t cache_read_32(uint32_t addr){
    uint32_t offsetWord = (addr & 0x0000000C) >> 2;
    ...
    return L1Cache.blocks[index].words[offsetWord];
}

void cache_write_32(uint32_t addr, uint32_t dataNew){
    uint32_t offsetWord = (addr & 0x0000000C) >> 2;
    uint32_t index = (addr & 0x000000F0) >> 4;
    uint32_t tag = (addr & 0xFFFFFF00) >> 8;
    uint32_t data;
    ...
    L1Cache.blocks[index].words[offsetWord] = data;
...
void MEM()
{
    ...
    switch(opcode){
        ...
        case 0x23: //LW
            ...
            break;
        case 0x28: //SB
            cache_write_32(EX_MEM.ALUOutput, EX_MEM.LMD);
            MEM_WB.RegWrite = 0;
            break;
        case 0x29: //SH
            cache_write_32(EX_MEM.ALUOutput, EX_MEM.LMD);
